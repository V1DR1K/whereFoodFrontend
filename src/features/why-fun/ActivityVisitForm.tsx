...
export function ActivityVisitForm({ activity, visit, onClose, onSaved }: { activity: Activity; visit?: ActivityVisit; onClose: () => void; onSaved: (visit: ActivityVisit) => void }) {
  ...
  const [scheduledAt, setScheduledAt] = useState(visit?.scheduledAt?.slice(0, 16) ?? localDateTime());
  ...
  return <Modal onClose={onClose} confirmDiscard pending={mutation.isPending || remove.isPending}><form onSubmit={(event) => { event.preventDefault(); mutation.mutate(); }}><p className="eyebrow">{visit ? "EDITAR SALIDA" : "NUEVA SALIDA"}</p><h2>{activity.name}</h2><label>Fecha y hora<input type="datetime-local" required value={scheduledAt} onChange={(event) => setScheduledAt(event.target.value)} /></label><button className="main-button" disabled={mutation.isPending || remove.isPending}>{mutation.isPending ? "Guardando…" : visit ? "✓ Guardar salida" : "＋ Registrar salida"}</button>{visit && <button className="danger-button" type="button" onClick={() => setConfirming(true)}>× Borrar salida</button>}{(mutation.error || remove.error) && <p className="form-error">{(mutation.error || remove.error)!.message}</p>}</form></Modal>;
}
