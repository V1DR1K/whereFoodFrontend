    ...
    <section className="fun-filter">
      ...
      {showAll && (
        <Modal onClose={() => setShowAll(false)}>
          ...
          <div className="chips fun-filter-dialog">
            <button
              className={!selected ? "selected" : ""}
              onClick={() => choose()}
            >
...
export function WhyFunPage() {
  ...
  const categories = useQuery({
    queryKey: ["fun-categories"],
    queryFn: getFunCategories,
  });
  ...
  const venues = useInfiniteQuery({
    queryKey: ["fun-venues", categoryId, subcategoryId],
    queryFn: ({ pageParam }) =>
      getFunVenues({ categoryId, subcategoryId, cursor: pageParam }),
  ...
  return (
    <>
      <section className="fun-hero">
        <div>
          <p className="eyebrow">SALIR A JUGAR</p>
          <h1>
            ¿Qué vamos a<br />
            <em>hacer </em> hoy?
          </h1>
          <p>
            Los lugares para competir, jugar, explorar y volver a pasarla bien.
          </p>
        </div>
        <div className="fun-hero-art">
          🎲<span>✦</span>
          <b>🕹️</b>
        </div>
      </section>
      <nav className="quick-nav quick-nav-action">
        <button className="add-fun-button" onClick={() => setCreating(true)}>
          <span>＋</span>
          <span>
            <small>NUEVO PLAN</small>Agendar salida
          </span>
          <b>🎯</b>
        </button>
      </nav>
      ...
      <section className="fun-section">
        <div className="section-title">
          <div>
            <p className="eyebrow">PARA SALIR</p>
            <h2>Planes guardados</h2>
          </div>
          <strong>{list.length} lugares</strong>
        </div>
        {venues.isError ? (
          <p className="form-error">{venues.error.message}</p>
        ) : list.length ? (
          <div className="fun-grid">
            {list.map((venue) => (
              <FunVenueCard key={venue.id} venue={venue} />
            ))}
          </div>
        ) : (
          !venues.isLoading && (
            <p className="empty-state">
              Todavía no guardaron ninguna salida. Agenden la primera.
            </p>
          )
        )}
        <LoadMore
          enabled={venues.hasNextPage}
          onClick={() => venues.fetchNextPage()}
          loading={venues.isFetchingNextPage}
          label="Ver más planes"
        />
      </section>
      {creating && <FunVenueForm onClose={() => setCreating(false)} />}
    </>
  );
}
