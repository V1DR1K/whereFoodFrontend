...
export function CookingForm({ recipe, cooking, onClose, onSaved }: { recipe: Recipe; cooking?: Cooking; onClose: () => void; onSaved: (cooking: Cooking) => void }) {
  ...
  if (confirming && cooking) return <ConfirmDialog title="¿Borrar esta cocinada?" message="También se eliminarán sus fotos y reseñas." confirmLabel="Borrar cocinada" pending={remove.isPending} onClose={() => setConfirming(false)} onConfirm={() => remove.mutate()} />;
