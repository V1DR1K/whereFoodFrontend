...
function RecipeSection({
  ...
  home: Home;
  onAdd: (home: Home) => void;
  onEdit: (recipe: HomeRecipe) => void;
  onRepeat: (recipe: HomeRecipe) => void;
}) {
  const recipes = useQuery({
    queryKey: ["home-recipes", home],
    queryFn: () => getHomeRecipes(home),
  });
  ...
  return (
    <section className="home-recipe-section">
      <div className="section-title">
        <div>
          <p className="eyebrow">
            {home === "TOMAS" ? "🏠 CASA TOMÁS" : "🏡 CASA AVRIL"}
          </p>
          <h2>En casa de {homeName(home)}</h2>
        </div>
      </div>
      <button
        className="add-cook-button home-recipe-add"
        onClick={() => onAdd(home)}
      >
        <span className="add-cook-icon">＋</span>
        <span>
          <small>RECETA CASERA</small>Anotar
        </span>
        <b>🍳</b>
      </button>
      ...
      {!recipes.isLoading && !list.length && (
        <p className="empty-state">Todavía no anotaron recetas en esta casa.</p>
      )}
    </section>
  );
}
...
function RecipeCard({
  ...
  return (
    <article className="home-recipe-card">
      {image ? (
        <img src={mediaUrl(image)} alt={`Foto de ${recipe.name}`} />
      ) : (
        <div className="home-recipe-card__empty">🍳</div>
      )}
      <div className="home-recipe-card__body">
        <div className="home-recipe-card__heading">
          <div>
            <p>
              {mealName(recipe.mealType)} · {dateLabel(recipe.preparedOn)}
            </p>
            <h3>{recipe.name}</h3>
          </div>
          <button
    ...
    <section className="home-recipes">
      ...
      {repeating && (
        <HomeRecipeForm
          home={repeating.home}
          copyOf={repeating}
          onClose={() => setRepeating(undefined)}
        />
      )}
    </section>
  );
